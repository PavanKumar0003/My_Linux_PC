...
bool my_robot_interfaces__srv__set_led__request__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // lednumber
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      if (!rosidl_runtime_c__int64__Sequence__init(&(ros_message->lednumber), size)) {
        PyErr_SetString(PyExc_RuntimeError, "unable to create int64__Sequence ros_message");
        PyBuffer_Release(&view);
        Py_DECREF(field);
        return false;
      }
      int64_t * dest = ros_message->lednumber.data;
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'lednumber'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
PyObject * my_robot_interfaces__srv__set_led__request__convert_to_py(void * raw_ros_message)
{
  ...
  {  // lednumber
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "lednumber");
    if (!field) {
      return NULL;
    }
    ...
    if (ros_message->lednumber.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int64_t * src = &(ros_message->lednumber.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->lednumber.size * sizeof(int64_t));
      assert(data != NULL);
